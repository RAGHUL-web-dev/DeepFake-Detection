...
import { User, LayoutDashboard, ShieldAlert, LogOut, ChevronDown, ChevronRight } from 'lucide-react';
...
function Navbar() {
    ...
    const products = [
        ...
        { id: 'audio', name: 'Audio Authentication', href: '/deepfakeVerification/voiceDetection', description: 'Verify the authenticity of audio recordings by analyzing digital signatures and background noise consistency.' },
    ];
    ...
    // Set initial hovered product
    useEffect(() => {
        if (isProductDropdownOpen && !hoveredProduct) {
            setHoveredProduct(products[0]);
        }
    }, [isProductDropdownOpen, products, hoveredProduct]);
    ...
    const productDropdownRender = () => (
        <div 
            ...
            className="fixed left-0 right-0 top-16 bg-dark/95 backdrop-blur-2xl border-b border-white/5 shadow-2xl overflow-hidden z-50"
            style={{
                animation: 'slideDown 0.3s ease-out forwards',
                transformOrigin: 'top'
            }}
        >
            <div className="max-w-7xl mx-auto px-6 lg:px-8 py-10 flex min-h-[450px]">
                {/* Left side: Links */}
                <div className="w-1/3 border-r border-white/5 pr-8 flex flex-col gap-1 overflow-y-auto">
                    {products.map((product) => (
                        <Link
                            ...
                            className={`px-5 py-4 rounded-xl transition-all duration-300 flex items-center justify-between group ${
                ...
                <div className="flex-1 pl-16 flex flex-col justify-center max-w-3xl">
                    {hoveredProduct && (
                        <div className="space-y-6 animate-fadeIn" key={hoveredProduct.id}>
    ...
    const authItems = [
        {
            key: 'profile',
            label: (
                <Link href={isAdmin ? "/admin/profile" : "/dashboard/profile"} className="flex items-center gap-2 px-1 py-1">
        ...
        },
        {
            key: 'logout',
            label: (
                <div onClick={() => logoutMutate()} className="flex items-center gap-2 px-1 py-1 text-red-500 cursor-pointer">
    ...
    return (
        <>
            ...
            <nav ref={navRef} className="fixed top-0 left-0 right-0 z-50 bg-dark/95 backdrop-blur-xl border-b border-white/5">
                        ...
                        <div className="hidden lg:flex items-center gap-1">
                            ...
                            <div className="relative">
                                <button
                                    ...
                                    className={`px-4 py-2 text-sm font-medium transition-colors duration-200 flex items-center gap-1 group relative ${
                                        isProductDropdownOpen ? 'text-white' : 'text-white/70 hover:text-white'
                                    }`}
                                >
                                    Product
                                ...
                                {isProductDropdownOpen && productDropdownRender()}
                            </div>
                                ...
                                <div className="mt-4 pt-4 border-t border-white/5 flex flex-col gap-2">
                                    {isAuthenticated ? (
                                        <>
                                            ...
                                            <button
                                                onClick={() => { logoutMutate(); setIsMenuOpen(false); }}
